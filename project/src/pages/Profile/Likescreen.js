  ...
  const fetchTweets = useCallback(
    async pageNum => {
      ...
      try {
        ...
        const responseTweet = await axios.post(`${serverUrl}/liked-posts`, {
          token,
        });
        const dataTweet = responseTweet.data.data;
  
        const sortedTweets = dataTweet
          .filter(post => post.user !== null)
          .sort((a, b) => {
            // Cek apakah post sudah di-like oleh user
            const aLiked = a.likes.some(like => like._id === idUser);
            const bLiked = b.likes.some(like => like._id === idUser);
            
            if (aLiked && bLiked) {
              const aLikeTime = a.likes.find(like => like._id === idUser).created_at;
              const bLikeTime = b.likes.find(like => like._id === idUser).created_at;
              return new Date(bLikeTime) - new Date(aLikeTime);
            }
        ...
        const formattedTweets = sortedTweets.map(post => {
          const totalComments =
