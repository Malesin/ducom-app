...
const ReportsManagement = () => {
  ...
  const FetchReports = async () => {
    ...
    try {
      const response = await axios.post(`${serverUrl}/show-reports`, {
        token: token,
      });
      const resp = response.data.data;
      const formatted = resp.map(report => {
        const userId =
          report.reportedEntity?.user?._id ??
          report.reportedEntity?._id ??
          report.reportedEntity?.user ??
          '';
        const name =
          report.reportedEntity?.user?.name ??
        ...
        const profilePicture =
          report.reportedEntity?.user?.profilePicture ??
          report.reportedEntity?.profilePicture ??
          '';
